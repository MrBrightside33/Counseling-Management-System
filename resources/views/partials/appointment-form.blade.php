...
<form data-appointment-form data-store-url="{{ route('appointments.store') }}" method="POST" action="{{ $formAction }}"
    class="space-y-5">
    @csrf
    @if ($isEdit)
        @method('PUT')
    ...
    <input type="hidden" name="appointment_record_id" id="appointment_record_id"
        value="{{ old('appointment_record_id', $appointment->id ?? '') }}">

    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
        <div>
            <label for="appointment_student_id" class="mb-1 block text-sm font-medium text-gray-700">Student *</label>
            <select id="appointment_student_id" name="student_id" required
            ...
            @if ($students->isEmpty())
                <p class="mt-1 text-xs text-amber-600">
                    <a href="{{ route('students.index', ['add' => 1]) }}" class="font-medium underline">Add a student</a>
                    first.
                </p>
            @endif
        </div>
        ...
        <div>
            <label for="appointment_counselor_id" class="mb-1 block text-sm font-medium text-gray-700">Counselor
                *</label>
            <select id="appointment_counselor_id" name="counselor_id" required
                class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-[#140DED] focus:outline-none focus:ring-1 focus:ring-[#140DED]">
    ...
    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
        <div>
            ...
            <input id="appointment_date" name="date" type="date" required min="{{ date('Y-m-d') }}"
    ...
    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
        <div>
            <label for="appointment_type" class="mb-1 block text-sm font-medium text-gray-700">Type of counseling
                *</label>
            <select id="appointment_type" name="type" required
                class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-[#140DED] focus:outline-none focus:ring-1 focus:ring-[#140DED]">
                <option value="">Select type</option>
                @foreach ($types as $type)
                    <option value="{{ $type }}" @selected(old('type', $appointment->type ?? '') === $type)>{{ $type }}
                    </option>
                @endforeach
            </select>
            @error('type')
    ...
    <div>
        ...
        <textarea id="appointment_notes" name="notes" rows="3"
            ...
            placeholder="Session details or reminders...">{{ old('notes', $appointment->notes ?? '') }}</textarea>
        @error('notes')
            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
        @enderror
    </div>
    ...
    <div class="flex flex-col gap-3 border-t border-gray-100 pt-4 sm:flex-row sm:items-center">
        <button type="submit" data-appointment-submit
        ...
        @if ($returnTo === 'appointments')
            <button type="button" data-appointment-modal-close
