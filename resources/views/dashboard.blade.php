    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                ...
                <p class="mt-2 text-3xl font-bold text-green-600">{{ $scheduledAppointments }}</p>
            </div>
            <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
            <div class="rounded-xl border border-gray-200 bg-white shadow-sm">
                <div class="flex items-center justify-between border-b border-gray-100 px-6 py-4">
                    ...
                    <a href="{{ route('appointments.index') }}" class="text-sm font-medium text-[#140DED] hover:underline">View all</a>
                </div>
                <div class="divide-y divide-gray-100 p-6">
                    @forelse ($recentAppointments as $appointment)
                        <div class="flex gap-4 py-4 first:pt-0 last:pb-0">
                            <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#140DED]/10 text-[#140DED]">
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                @endphp
                                <span class="mt-2 inline-flex rounded-full px-2 py-0.5 text-xs font-medium capitalize {{ $statusClass }}">
                    ...
                    @empty
                        <p class="text-sm text-gray-500">
                            ...
                            <a href="{{ route('appointments.index', ['schedule' => 1]) }}" class="font-medium text-[#140DED] hover:underline">Schedule one</a>
                        </p>
                    @endforelse
                </div>
            </div>
            ...
            <div class="rounded-xl border border-gray-200 bg-white shadow-sm">
                ...
                <div class="divide-y divide-gray-100 p-6">
                    @forelse ($counselors->take(4) as $counselor)
                        <div class="flex gap-4 py-4 first:pt-0 last:pb-0">
                            <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-purple-100 text-purple-600">✓</div>
                            <div>
                                <p class="font-medium text-gray-900">{{ $counselor->name }}</p>
                                <p class="text-sm text-gray-600">{{ $counselor->specialization }}</p>
                                <p class="mt-1 text-xs text-gray-500">{{ $counselor->availability ?? 'Availability not set' }}</p>
                            </div>
                        </div>
                    @empty
