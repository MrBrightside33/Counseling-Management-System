...
@section('content')
    <div class="space-y-6">
        <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
            ...
            <button type="button" data-student-modal-open
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
            ...
            <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
                <div class="text-2xl font-bold text-purple-100">{{ $allStudents->where('status', 'active')->count() }}</div>
                <div class="text-sm text-gray-600">Active Students</div>
            </div>
            <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
                <div class="text-2xl font-bold text-red-600">{{ $allStudents->pluck('program')->unique()->count() }}</div>
        ...
        <div class="rounded-xl border border-gray-200 bg-white shadow-sm">
            <div class="flex flex-col gap-4 border-b border-gray-100 p-6 sm:flex-row sm:items-center sm:justify-between">
                ...
                <form method="get" action="{{ route('students.index') }}" class="w-full sm:w-64">
                    <input type="search" name="q" value="{{ $q }}" placeholder="Search students..."
                        ...
                        @forelse ($students as $student)
                                            <tr class="border-b border-gray-100">
                                                ...
                                                <td class="py-3 pr-4">
                                                    ...
                                                    @endphp
                             <span
                                                        class="inline-flex rounded-full px-2 py-0.5 text-xs font-medium capitalize {{ $statusClass }}">
                                                        {{ $student->status }}
                                                    </span>
                                                </td>
                                                ...
                                                <td class="py-3 text-right">
                                                    <div class="flex items-center justify-end gap-1.5">
                                                        <x-icon-button icon="edit" label="Edit {{ $student->name }}" type="button"
                                                            data-student-edit data-id="{{ $student->id }}"
                                                            data-update-url="{{ route('students.update', $student) }}"
                                                            data-name="{{ $student->name }}" data-student-id="{{ $student->student_id }}"
                                                            data-email="{{ $student->email }}" data-program="{{ $student->program }}"
                                                            data-year-level="{{ $student->year_level }}" data-status="{{ $student->status }}" />
                                                        <form method="POST" action="{{ route('students.destroy', $student) }}" class="inline"
                                                            ...
                                                            @csrf
                                                            @method('DELETE')
                                                            <x-icon-button icon="delete" label="Delete {{ $student->name }}" variant="danger"
                                                                type="submit" />
                                                        </form>
                                                    </div>
                                                </td>
                            ...
                            <tr>
                                <td colspan="7" class="py-8 text-center text-gray-500">
                                    ...
                                    @else
                                        No students yet.
                                        <button type="button" data-student-modal-open
                                            class="font-medium text-[#140DED] hover:underline">Add your first student</button>
                                    @endif
                                </td>
                            </tr>
                        @endforelse
